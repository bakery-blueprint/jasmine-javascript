...
```javascript
    ...
    // DMZ 객체 생성
    var o = Object.create(null);    // {}와 비슷하나 Object.prototype 으로 위임하지 않으므로 더 텅빈 객체임
...
```javascript
    ...
    (p.foo = o.foo)(); //2 
```
-  (p.foo = o.foo)의 결과값은 원함수 객체의 레퍼런스로 foo()가 되며 기본바인딩 규칙이 적용된다.
...
```javascript
    ...
    setTimeout(obj2.foo, 10);   //name : obj -> 소프트 바인딩 적용
```
