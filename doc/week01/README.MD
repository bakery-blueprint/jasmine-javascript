...
> 배열 : 문자열, 숫자, 객체, 다른 배열까지 어떤 타입의 값을 담을 수 있음
...
```javascript
    ...
    // 빈 배열을 다룰 때 유의!
    var a = [];                 // 미리 크기를 정하지 않고도 선언할 수 있음
    a[1];                       //undefined 지만  a[1] = 'undefined'는 아님 ***** 질문!!!
...
```javascript
    ...
    출처 : https://haenny.tistory.com/86

```

> 문자열 : 문자열은 배열과 겉모습이 닮은 유사배열, 하지만 배열은 아님
- 배열과 공통점 : length 프로퍼티, indexOf() 메서드, concat() 메서드 
- 배열과 차이점 : 문자열은 불변값, 배열은 가변값으로 문자열 메서드는 그 내영을 바로 변경하지 않고 항상 새로운 문자열을 생성한 후 반환, 배열 메서드는 그 자리에서 곧바로 원소를 수정
...
> 숫자 : javascript 숫자 타입은 Number가 유일하며 정수, 부동 소숫점 숫자를 모두 아우름
...
> 값 복사 vs 레퍼런스 복사 
자바스크립트에는 값 또는 레퍼런스의 할딩 및 전달을 제어하는 구무암시가 전혀 없음. 값의 타입만으로 값-복사, 레퍼런스-복사 둘중에 한 쪽이 결정됨. 값의 타입을 보고 엔진의 재량으로 결정
- null, undefined, String, number, boolean :  값 복사
- 객체나 함수 등 합성 값은 할당/전달 시  : 레퍼런스-복사
...
## 네이티브
> 네이티브란 특정 환경에 종속되지 않은 ECMAScript 명세의 내장 객체
