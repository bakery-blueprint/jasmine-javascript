...
typeof [1,2,3]              // object
```
- 자바스크립트는 동적 타입 언어로, 변수의 타입 지정없이 값이 할당되는 과정에서 자동으로 변수의 타입이 결정됨. 즉, 변수는 고정된 타입이 없며, 같은 변수에 여러 타입의 값을 자유롭게 할당할 수 있음
...
1. 조건1 : 반드시 length가 필요하다.
2. 조건2 : index의 번호가 0번부터 시작해서 1씩 증가해야한다. 
3. 구분 : Array.isArray();
